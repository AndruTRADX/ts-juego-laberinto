...
`);B.push(`
  ...
  XX-----OXX
  XXXXXXXXXX
`);function C(n){const e=Math.floor(n/1e3/60).toString().padStart(2,"0"),c=(Math.floor(n/1e3)%60).toString().padStart(2,"0"),r=Math.floor(n%1e3/100).toString().padStart(1,"0");return`${e}:${c}:${r}`}let t={x:void 0,y:void 0},f={x:void 0,y:void 0},T=[],s,X;const D=10;let h=0,m=3,l,y,g=!1,w;const d=document.querySelector("#game"),a=d.getContext("2d"),_=document.getElementById("up"),Y=document.getElementById("left"),z=document.getElementById("right"),F=document.getElementById("down"),$=document.getElementById("start-game"),p=document.getElementById("modal-start-game"),x=document.getElementById("modal-game-over"),O=document.getElementById("modal-game-win"),q=document.getElementById("finish"),M=document.getElementById("win-message"),K=document.getElementById("start-game-after"),U=document.getElementById("no-continue"),A=document.getElementById("modal-counter"),R=document.getElementById("counter"),V=document.getElementById("reset"),j=document.getElementById("lives"),Q=document.getElementById("time"),J=document.getElementById("lives-2"),Z=document.getElementById("time-2"),ee=document.getElementById("record"),te=document.getElementById("record-2");_.addEventListener("click",G);Y.addEventListener("click",b);z.addEventListener("click",H);F.addEventListener("click",P);function G(){t.y!==void 0&&(Math.ceil(t.y-X)<X||(t.y-=X,u()))}function b(){t.x!==void 0&&(Math.ceil(t.x-X)<X||(t.x-=X,u()))}function H(){t.x!==void 0&&(Math.floor(t.x+X)>s||(t.x+=X,u()))}function P(){t.y!==void 0&&(Math.floor(t.y+X)>s||(t.y+=X,u()))}window.addEventListener("keydown",ne);function ne(n){const e=n.key;e==="ArrowUp"||e==="w"||e==="W"?G():e==="ArrowLeft"||e==="a"||e==="A"?b():e==="ArrowRight"||e==="d"||e==="D"?H():(e==="ArrowDown"||e==="s"||e==="S")&&P()}function S(){h=0,m=3,l=void 0,clearInterval(y),W()}$.addEventListener("click",()=>{g=!0,S()});V.addEventListener("click",()=>S());window.addEventListener("resize",()=>S());function W(){if(window.innerWidth<500?w=.875:w=.65,g===!0){window.innerHeight>window.innerWidth?s=window.innerWidth*w:s=window.innerHeight*w,Number(s.toFixed(0)),d==null||d.setAttribute("width",String(s)),d==null||d.setAttribute("height",String(s)),X=s/D,t.x=void 0,t.y=void 0,p&&(p.style.display="none"),A.classList.remove("hidden");let n=2;R.innerText="3";const e=setInterval(()=>{R.innerText=String(n),n--,n<0&&(clearInterval(e),A.classList.add("hidden"),u())},1e3)}}function u(){if(g==!0&&a){const n=B[h];if(!n){Xe();return}l||(l=Date.now(),clearInterval(y),y=setInterval(ce,100),k());const c=n.trim().split(`
